...
internal class WriteSourceFileToStorageTask : BuTaskV2
{
    ...
    public WriteSourceFileToStorageTask(
        ...
        base(services.CommonServices.Log, events, 
            sourceItem != null 
            ? string.Format(BUtil.Core.Localization.Resources.File_Saving, string.Join(", ", storageFiles.Select(x => SourceItemHelper.GetFriendlyFileName(sourceItem, x.FileState.FileName))))
            : string.Format(BUtil.Core.Localization.Resources.File_Saving, fileTitle))
    {
        _services = services;
