...
internal class SynchronizationServices : IDisposable
{
    ...
    public SynchronizationServices(ILog log, string taskName, string syncFolder, IStorageSettingsV2 remoteStorageSettings, bool autodetectConnectionSettings)
    {
        Log = log;
        CachedHashStoreService = new CashedHashStoreService();
        HashService = new CachedHashService(CachedHashStoreService);
        LocalStateService = new SynchronizationLocalStateService(HashService, taskName, syncFolder);
        ...
        ActualFilesService = new SynchronizationActualFilesService(HashService, syncFolder);
    }
